{ lib, stdenv, python3, hyprwhspr-src, wl-clipboard, wtype, ydotool, pciutils, writeShellScriptBin }:
...
let
  ...
  ydotoolWrap = writeShellScriptBin "ydotool" ''
...
stdenv.mkDerivation {
  pname = "hyprwhspr";
  # Muss zum gepinnten Tag in flake.nix (hyprwhspr-src) passen.
  version = "1.43.0";
